...
// Módulo legado de despesas manuais + motor de travas/limites (fin_despesas /
// fin_categorias / fin_limites), portado do sistema antigo. Preservado numa
// rota própria (fora do menu) — roadmap para reintegrar sobre os pagamentos.
// A rota /financeiro agora é o detalhamento do plano de pagamentos.
export const Route = createFileRoute("/_authenticated/financeiro-despesas")({
  head: () => ({ meta: [{ title: "Controle de Despesas (travas) — HKTC" }] }),
...
async function fetchFinanceiro() {
  ...
  return {
    ...
    limites: ((lim.data ?? []) as unknown[]).map((l) => {
      ...
      return {
        ...row,
        alertas_pct: Array.isArray(row.alertas_pct) ? (row.alertas_pct as number[]) : [80],
      } as Limite;
    }),
  };
}
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            ...
            <p>
              ...
              <code>supabase/migrations/20260723000000_financeiro_module.sql</code> no projeto
              Supabase.
            </p>
          </CardContent>
        </Card>
      ...
      <Dialog open={!!confirm} onOpenChange={(o) => !o && setConfirm(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Este lançamento ultrapassa{" "}
              {confirm?.avisos.length === 1 ? "um limite flexível" : "alguns limites flexíveis"}.
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() =>
                cat &&
                confirm &&
                inserir({ cents: confirm.cents, natureza: cat.natureza, justificativa })
              }
            >
              Lançar assim mesmo
            </Button>
  ...
  const salvar = async () => {
    ...
    if (edit.id) {
      ...
      ({ error } = await supabase
        .from("fin_categorias")
        .insert({ ...row, user_id: sess.user?.id }));
    }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        {limites.length === 0 && (
          <p className="text-sm text-muted-foreground">
            Nenhuma trava. Trava <b>rígida</b> bloqueia o lançamento; <b>flexível</b> avisa e
            permite lançar com justificativa — e vira rígida acima do % informado.
          </p>
        )}
        {limites.map((l) => (
          <div key={l.id} className="flex items-center gap-3 rounded-md border p-2">
            ...
            {l.modo === "soft" && l.limiar_hard_pct != null && (
              <span className="text-xs text-muted-foreground">
                bloqueia acima de {l.limiar_hard_pct}%
              </span>
            )}
            <div className="ml-auto flex gap-1">
              <Button
          ...
          {edit && (
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <Select
                  value={edit.escopo}
                  onValueChange={(v) => setEdit({ ...edit, escopo: v as Escopo })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              {edit.escopo === "categoria" && (
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={edit.alvo ?? ""}
                    onValueChange={(v) => setEdit({ ...edit, alvo: v })}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Escolha…" />
                    </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={edit.modo}
                  onValueChange={(v) => setEdit({ ...edit, modo: v as ModoTrava })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="soft">
                      Flexível (avisa e permite com justificativa)
                    </SelectItem>
