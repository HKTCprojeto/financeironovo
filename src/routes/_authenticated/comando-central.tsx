...
export const Route = createFileRoute("/_authenticated/comando-central")({
  // Comando Central (template Lovable/Lívia) — preservado numa rota própria.
  // A home (/) agora é o Painel de Pagamentos. Esta tela segue no roadmap.
...
function useInsights() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "marcos_insights" },
        (p) => {
...
function ComandoCentral() {
  ...
  const [simResult, setSimResult] = useState<null | {
    delta_balance_brl: number;
    new_balance_30d: number;
  }>(null);
        ...
        <div className="border border-destructive/40 bg-destructive/10 text-destructive-foreground rounded-md px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-destructive">
              Modo crise ativado — {criticalInsights.length} alerta(s) crítico(s)
            </p>
        ...
        <div className="border border-primary/40 bg-primary/5 rounded-md px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex-1 text-sm">
            ...
            <p className="text-xs text-muted-foreground">
              KPIs ficarão zerados até conectar pelo menos uma.
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={dismissNoInteg}>
              Fechar por hoje
            </Button>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <span
                className={`h-2 w-2 rounded-full ${realtimeOk ? "bg-emerald-500 animate-pulse" : "bg-destructive"}`}
              />
                    ...
                    <div className="min-w-0">
                      ...
                      {d.days_overdue !== undefined && (
                        <p className="text-xs text-muted-foreground font-mono">
                          {d.days_overdue}d em atraso
                        </p>
                      )}
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold tabular-nums">
                        {formatCurrencyBRL(d.brl)}
                      </p>
                      <button
              ...
              {isLoading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data.cash_projection_90d}>
                    ...
                    />
                    {goals
                      .filter((g) => g.metric === "balance_brl")
                      .map((g) => (
                        <ReferenceLine
                          key={g.id}
                          y={Number(g.target_value)}
                          stroke="var(--color-chart-3)"
                          strokeDasharray="4 4"
                          label={{ value: "Meta", fill: "var(--color-chart-3)", fontSize: 10 }}
                        />
                      ))}
                  </LineChart>
                </ResponsiveContainer>
          ...
          <CardContent className="h-72">
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={data.by_channel_revenue_30d}
                    dataKey="brl"
                    nameKey="channel"
                    innerRadius={50}
                    outerRadius={90}
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            <ScenarioSlider
              ...
              hint={
                data
                  ? `+${formatCurrencyBRL((data.kpis.overdue_total_brl * scenario.collect) / 100)}`
                  : "—"
              }
            />
            <ScenarioSlider
            ...
            <ScenarioSlider
              ...
              hint={
                data
                  ? `economia: ${formatCurrencyBRL((data.kpis.payables_30d_brl * scenario.cut) / 100)}`
                  : "—"
              }
            />
                ...
                {simResult ? (
                  <span className="font-mono tabular-nums">
                    ...
                    <span className="text-emerald-500 font-semibold">
                      {formatCurrencyBRL(simResult.new_balance_30d)}
                    </span>{" "}
                    <span className="text-emerald-500 text-xs">
                      (+{formatCurrencyBRL(simResult.delta_balance_brl)})
                    </span>
                  </span>
                ) : (
              ...
              {isLoading ? (
                ...
                <div className="flex flex-wrap gap-3">
                  {(data?.integrations_health ?? []).map((h) => (
                    <div
                      key={h.name}
                      className="flex items-center gap-2 text-xs font-mono border border-border rounded px-3 py-1.5"
                    >
                      {h.status === "ok" ? (
                      ...
                      <span className="text-muted-foreground">
                        {h.last_sync ? formatRelative(h.last_sync) : "—"}
                      </span>
                    </div>
                  ))}
                </div>
    ...
    <div className="bg-popover border border-border rounded-md px-3 py-2 shadow-lg text-xs font-mono">
      ...
      {payload.map((p, i) => (
        <div key={i} className="flex items-center gap-2">
          <span>
            {p.name ??
              (p.payload as { channel?: string; stage?: string })?.channel ??
              (p.payload as { stage?: string })?.stage ??
              ""}
          </span>
    ...
    >
      ...
      <CardContent className="space-y-2">
        {loading ? (
          ...
          <div
            className={`text-2xl font-semibold tabular-nums font-mono ${warn ? "text-amber-400" : ""}`}
          >
            {override !== undefined ? (
        ...
        {subtext && !loading && <p className="text-[11px] text-muted-foreground">{subtext}</p>}
        {insight && (
          <div
            className={`text-xs flex items-start gap-1.5 ${isCritical ? "text-destructive" : "text-muted-foreground"}`}
          >
            {isCritical ? (
      ...
      <CardContent className="space-y-3">
        {isLoading ? (
          ...
          <div className="text-2xl font-semibold tabular-nums font-mono">
            <span className={data.ok === data.total ? "text-emerald-500" : "text-amber-500"}>
              {data.ok}
            </span>
...
function CostBudgetCard() {
  ...
  const textColor =
    pct >= 90 ? "text-destructive" : pct >= 70 ? "text-amber-500" : "text-emerald-500";
      ...
      <Card className="max-w-md w-full">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Antes de ver o Comando Central você precisa registrar uma instância e conectar suas
            integrações.
          </p>
          <Link to="/onboarding">
